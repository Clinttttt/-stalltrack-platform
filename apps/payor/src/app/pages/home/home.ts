 ...
 * the Blazor portal unreadable for a market payor, and it is not repeated here.
 */
@Component({
  selector: 'app-home',
  template: `
    <main class="p-shell px-5 pb-10">
      ...
      @if (loading()) {
        ...
        <section class="mt-6 rounded-2xl border border-line bg-navy p-5 text-white shadow-soft">
          <p class="text-[10px] font-semibold uppercase tracking-[0.14em] text-white/70">Total outstanding</p>
          <p class="mt-1 font-display text-3xl">{{ peso(total()) }}</p>
          ...
          <article class="mt-3 rounded-2xl border border-line bg-white p-4 shadow-soft">
            <div class="flex items-start justify-between gap-3">
              ...
              <div class="shrink-0 text-right">
                <p class="font-display text-base text-navy">
                  {{ peso(a.isDailyBilled ? a.dailyRate : a.monthlyRate) }}
                </p>
            ...
            @if (a.outstandingBalance > 0) {
              <p class="mt-3 border-t border-line pt-3 text-xs text-muted">
                Outstanding <span class="font-semibold text-navy">{{ peso(a.outstandingBalance) }}</span>
              </p>
            }
          </article>
        } @empty {
